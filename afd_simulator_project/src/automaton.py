...
class Automaton:
    ...
    def _re_union(self, a: str, b: str) -> str:
        if not a:
            return b
        if not b:
            return a
    ...
    def _re_concat(self, *parts: str) -> str:
        clean = []
        for part in parts:
            if not part:
                return ''
            if part in ('ε', EPSILON):
                continue
            if '|' in part and not (part.startswith('(') and part.endswith(')')):
                part = '(' + part + ')'
            clean.append(part)
        return ''.join(clean) if clean else 'ε'

    def _re_star(self, a: str) -> str:
        if not a or a in ('ε', EPSILON):
            return 'ε'
        if len(a) == 1:
            return a + '*'
        if a.endswith('*'):
    ...
    def dfa_to_regex_with_steps(self) -> Tuple[str, List[dict]]:
        """Convierte un AFD completo a ER usando eliminación de estados.
        Devuelve la ER final y una lista de pasos legibles para la interfaz.
        """
        if not self.is_dfa():
            raise ValueError('Para convertir a ER se necesita un AFD válido y completo. Usa “Completar AFD” si faltan transiciones.')
        start = '__INICIO__'
        final = '__FINAL__'
        states = [start] + sorted(self.states) + [final]
        R = {(i, j): '' for i in states for j in states}
        R[(start, self.initial_state)] = 'ε'
        for acc in self.accept_states:
            R[(acc, final)] = self._re_union(R[(acc, final)], 'ε')
        for src in sorted(self.states):
            for sym in sorted(self.alphabet):
                dst = next(iter(self.transitions[src][sym]))
                R[(src, dst)] = self._re_union(R[(src, dst)], sym)
        steps = []
        eliminables = sorted(self.states)
        for k in eliminables:
            before = {(i, j): v for (i, j), v in R.items() if v}
            updates = []
            loop = R[(k, k)]
            for i in states:
                if i == k:
                ...
                for j in states:
                    ...
                    if not rkj:
                        continue
                    candidate = self._re_concat(rik, self._re_star(loop), rkj)
                    old = R[(i, j)]
                    new = self._re_union(old, candidate)
                    if new != old:
                        R[(i, j)] = new
                        updates.append({'from': i, 'to': j, 'old': old or '∅', 'added': candidate, 'new': new})
            for i in states:
                R[(i, k)] = ''
                R[(k, i)] = ''
            after = {(i, j): v for (i, j), v in R.items() if v}
            steps.append({'eliminated': k, 'loop': loop or '∅', 'updates': updates, 'before': before, 'after': after})
        result = R[(start, final)] or '∅'
        return result, steps
