...
class AFDSimulatorApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Button(toolbar, text="AFD → ER", command=self.prepare_dfa_to_regex).pack(side="left", padx=10)
        ttk.Button(toolbar, text="Importar .jff AFD", command=self.import_jff_dfa).pack(side="left", padx=2)
        ...
        self.tab_regex_validators = ttk.Frame(self.notebook)
        ...
        self.notebook.add(self.tab_regex_validators, text="Validadores ER")
        ...
        self._build_regex_validators_tab()
    ...
    def _build_conversion_tab(self):
        controls = ttk.Frame(self.tab_conversion)
        controls.pack(fill="x", padx=8, pady=4)
        ttk.Button(controls, text="Preparar AFD → ER", command=self.prepare_dfa_to_regex).pack(side="left", padx=4)
        ttk.Button(controls, text="Siguiente eliminación", command=self.next_regex_step).pack(side="left", padx=4)
        self.regex_result_var = tk.StringVar(value="ER resultante: —")
        ttk.Label(controls, textvariable=self.regex_result_var, font=("Arial", 11, "bold")).pack(side="left", padx=12)

        upper = ttk.Frame(self.tab_conversion)
        upper.pack(fill="both", expand=True, padx=8, pady=6)
        left = ttk.LabelFrame(upper, text="Proceso paso a paso")
        left.pack(side="left", fill="both", expand=True, padx=4)
        right = ttk.LabelFrame(upper, text="Resumen / Comparación / ER")
        right.pack(side="left", fill="both", expand=True, padx=4)
        self.conversion_text = tk.Text(left)
        self.conversion_text.pack(fill="both", expand=True, padx=4, pady=4)
        self.comparison_text = tk.Text(right, height=12)
        self.comparison_text.pack(fill="both", expand=True, padx=4, pady=4)
        self.regex_canvas = tk.Canvas(right, bg="white", height=260)
        self.regex_canvas.pack(fill="both", expand=True, padx=4, pady=4)
    ...
    def _build_regex_validators_tab(self):
        frame = ttk.Frame(self.tab_regex_validators)
        frame.pack(fill="both", expand=True, padx=10, pady=10)
        top = ttk.LabelFrame(frame, text="Aplicación práctica de expresiones regulares")
        top.pack(fill="x", pady=6)
        self.validator_cases = {
            "Correo electrónico": {
                "pattern": r"^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$",
                "hint": "Debe tener usuario, @, dominio y terminación como .com o .mx.",
                "states": ["q0", "usuario", "@", "dominio", "punto", "final"],
                "edges": [("q0","letra/número","usuario"),("usuario","@","@"),("@","dominio","dominio"),("dominio",".","punto"),("punto","letras","final")]
            },
            "Teléfono nacional MX": {
                "pattern": r"^(\+52\s?)?(\d{2,3}[\s-]?)?\d{3,4}[\s-]?\d{4}$",
                "hint": "Usa solo números, espacios o guiones. Ejemplo: +52 55 1234 5678.",
                "states": ["q0", "+52", "lada", "parte1", "final"],
                "edges": [("q0","+52 opcional","+52"),("+52","lada","lada"),("lada","3-4 dígitos","parte1"),("parte1","4 dígitos","final")]
            },
            "URL": {
                "pattern": r"^(https?://)?(www\.)?[A-Za-z0-9.-]+\.[A-Za-z]{2,}(/[A-Za-z0-9._~:/?#\[\]@!$&'()*+,;=%-]*)?$",
                "hint": "Debe tener dominio y extensión. Puede iniciar con http:// o https://.",
                "states": ["q0", "protocolo", "dominio", "punto", "final", "ruta"],
                "edges": [("q0","http opcional","protocolo"),("protocolo","dominio","dominio"),("dominio",".","punto"),("punto","extensión","final"),("final","/ruta opcional","ruta")]
            },
            "Fecha": {
                "pattern": r"^((0?[1-9]|[12][0-9]|3[01])[-/](0?[1-9]|1[0-2])[-/]\d{4}|\d{4}[-/](0?[1-9]|1[0-2])[-/](0?[1-9]|[12][0-9]|3[01]))$",
                "hint": "Formatos aceptados: dd/mm/aaaa, dd-mm-aaaa, aaaa/mm/dd o aaaa-mm-dd.",
                "states": ["q0", "día/año", "sep1", "mes", "sep2", "final"],
                "edges": [("q0","día o año","día/año"),("día/año","/ o -","sep1"),("sep1","mes","mes"),("mes","/ o -","sep2"),("sep2","año o día","final")]
            },
            "Contraseña segura": {
                "pattern": r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).{8,}$",
                "hint": "Mínimo 8 caracteres, una mayúscula, una minúscula, un número y un símbolo.",
                "states": ["q0", "longitud", "mayús", "minús", "número", "final"],
                "edges": [("q0","8+ caracteres","longitud"),("longitud","mayúscula","mayús"),("mayús","minúscula","minús"),("minús","número/símbolo","número"),("número","cumple todo","final")]
            },
        }
        ttk.Label(top, text="Caso de uso:").grid(row=0, column=0, padx=4, pady=4, sticky="w")
        self.validator_combo = ttk.Combobox(top, state="readonly", values=list(self.validator_cases.keys()), width=28)
        self.validator_combo.grid(row=0, column=1, padx=4, pady=4)
        self.validator_combo.set("Correo electrónico")
        self.validator_combo.bind("<<ComboboxSelected>>", lambda _e: self.update_validator_regex())
        ttk.Label(top, text="Texto a validar:").grid(row=1, column=0, padx=4, pady=4, sticky="w")
        self.validator_input = ttk.Entry(top, width=42)
        self.validator_input.grid(row=1, column=1, padx=4, pady=4)
        ttk.Button(top, text="Validar", command=self.run_regex_validator).grid(row=1, column=2, padx=4, pady=4)
        ttk.Button(top, text="Ver AFD equivalente", command=self.draw_validator_dfa).grid(row=1, column=3, padx=4, pady=4)
        self.validator_regex_var = tk.StringVar()
        ttk.Label(top, textvariable=self.validator_regex_var, wraplength=1000).grid(row=2, column=0, columnspan=4, padx=4, pady=4, sticky="w")
        self.validator_result_var = tk.StringVar(value="Resultado: —")
        ttk.Label(top, textvariable=self.validator_result_var, font=("Arial", 12, "bold")).grid(row=3, column=0, columnspan=4, padx=4, pady=4, sticky="w")
        self.validator_text = tk.Text(frame, height=10)
        self.validator_text.pack(fill="x", pady=6)
        self.validator_canvas = tk.Canvas(frame, bg="white", height=300)
        self.validator_canvas.pack(fill="both", expand=True, pady=6)
        self.update_validator_regex()

    def update_validator_regex(self):
        case = self.validator_cases[self.validator_combo.get()]
        self.validator_regex_var.set(f"ER utilizada: {case['pattern']}")
        self.validator_result_var.set("Resultado: —")
        self.validator_text.delete("1.0", tk.END)
        self.validator_text.insert(tk.END, "Ingresa un texto y presiona Validar. También puedes ver el AFD equivalente simplificado.\n")
        self.draw_validator_dfa()

    def run_regex_validator(self):
        name = self.validator_combo.get()
        case = self.validator_cases[name]
        value = self.validator_input.get().strip()
        ok = re.fullmatch(case["pattern"], value) is not None
        self.validator_result_var.set(f"Resultado: {'VÁLIDO' if ok else 'NO VÁLIDO'}")
        self.validator_text.delete("1.0", tk.END)
        self.validator_text.insert(tk.END, f"Caso: {name}\n")
        self.validator_text.insert(tk.END, f"ER: {case['pattern']}\n")
        self.validator_text.insert(tk.END, f"Entrada: {value!r}\n")
        if ok:
            self.validator_text.insert(tk.END, "La entrada cumple con el patrón especificado.\n")
        else:
            self.validator_text.insert(tk.END, "La entrada no cumple con el patrón. Sugerencia: " + case['hint'] + "\n")
        self.draw_validator_dfa(highlight_final=ok)

    def draw_validator_dfa(self, highlight_final=False):
        case = self.validator_cases[self.validator_combo.get()]
        c = self.validator_canvas
        c.delete("all")
        states = case["states"]
        edges = case["edges"]
        w = max(c.winfo_width(), 900)
        y = 150
        gap = max(110, (w - 120) // max(1, len(states)-1))
        pos = {st: (60 + i*gap, y) for i, st in enumerate(states)}
        for src, label, dst in edges:
            x1,y1 = pos[src]; x2,y2 = pos[dst]
            c.create_line(x1+30, y1, x2-30, y2, arrow=tk.LAST, width=2)
            c.create_text((x1+x2)/2, y1-22, text=label)
        c.create_line(10, y, 30, y, arrow=tk.LAST, width=2)
        for st in states:
            x,y0 = pos[st]
            fill = "#d4edda" if (st == states[-1] and highlight_final) else "white"
            c.create_oval(x-30, y0-30, x+30, y0+30, width=2, fill=fill)
            if st == states[-1]:
                c.create_oval(x-23, y0-23, x+23, y0+23, width=2)
            c.create_text(x, y0, text=st)
        c.create_text(w/2, 35, text="AFD equivalente simplificado para visualizar la ER del caso seleccionado", font=("Arial", 12, "bold"))

    def import_jff_dfa(self):
        path = filedialog.askopenfilename(filetypes=[("JFLAP", "*.jff")])
        if not path:
            return
        try:
            a = load_automaton(path)
            if not a.visuals:
                a.layout_states()
            if not a.is_dfa():
                errors = a.validate_nfa_structure()
                if not errors:
                    errors = ["El archivo se importó, pero no es un AFD completo: debe tener exactamente una transición por símbolo para cada estado y no debe tener λ."]
                messagebox.showerror("AFD no válido", "\n".join(errors))
                return
            self.automaton = a
            self.populate_form_from_automaton()
            self.refresh_views()
            messagebox.showinfo("JFLAP", "Archivo .jff importado y verificado como AFD válido.")
        except Exception as e:
            messagebox.showerror("Error de importación", str(e))

    def prepare_dfa_to_regex(self):
        try:
            self.regex_result, self.regex_steps = self.automaton.dfa_to_regex_with_steps()
            self.regex_step_index = 0
            self.conversion_text.delete("1.0", tk.END)
            self.comparison_text.delete("1.0", tk.END)
            self.conversion_text.insert(tk.END, "Conversión AFD → ER por eliminación de estados\n\n")
            self.conversion_text.insert(tk.END, "AFD original visualizado a la derecha. Presiona 'Siguiente eliminación' para avanzar.\n\n")
            self.regex_result_var.set("ER resultante: en proceso")
            self.comparison_text.insert(tk.END, "Se agregó un estado inicial nuevo con ε al inicial original y un estado final nuevo con ε desde cada estado de aceptación.\n")
            self.draw_automaton(self.regex_canvas, self.automaton)
            self.notebook.select(self.tab_conversion)
        except Exception as e:
            messagebox.showerror("Error", str(e))

    def next_regex_step(self):
        if not self.regex_steps:
            messagebox.showinfo("AFD → ER", "Primero presiona 'Preparar AFD → ER'.")
            return
        if self.regex_step_index >= len(self.regex_steps):
            self.regex_result_var.set(f"ER resultante: {self.regex_result}")
            self.comparison_text.delete("1.0", tk.END)
            self.comparison_text.insert(tk.END, "Expresión regular final:\n\n" + self.regex_result + "\n")
            return
        step = self.regex_steps[self.regex_step_index]
        n = self.regex_step_index + 1
        self.conversion_text.insert(tk.END, f"Paso {n}: eliminar estado {step['eliminated']}\n")
        self.conversion_text.insert(tk.END, f"  Bucle del estado eliminado: {step['loop']}\n")
        if step['updates']:
            self.conversion_text.insert(tk.END, "  Nuevas conexiones generadas:\n")
            for up in step['updates'][:20]:
                self.conversion_text.insert(tk.END, f"    {up['from']} → {up['to']}: {up['old']} ∪ {up['added']} = {up['new']}\n")
            if len(step['updates']) > 20:
                self.conversion_text.insert(tk.END, f"    ... y {len(step['updates'])-20} actualizaciones más.\n")
        else:
            self.conversion_text.insert(tk.END, "  No se generaron nuevas conexiones.\n")
        self.conversion_text.insert(tk.END, "\n")
        self.conversion_text.see(tk.END)
        self.regex_step_index += 1
        remaining = [st['eliminated'] for st in self.regex_steps[self.regex_step_index:]]
        self.comparison_text.delete("1.0", tk.END)
        self.comparison_text.insert(tk.END, f"Estados pendientes por eliminar: {remaining}\n\n")
        if self.regex_step_index >= len(self.regex_steps):
            self.regex_result_var.set(f"ER resultante: {self.regex_result}")
            self.comparison_text.insert(tk.END, "Expresión regular final:\n\n" + self.regex_result + "\n")
